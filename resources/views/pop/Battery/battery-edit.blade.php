...
<body>
        ...
        <main class="main-content">
            ...
            <div class="rform-content">
                ...
                <div class="rform-header-bar">
                    <a href="{{ route('batteries.show', [$pop->id, $battery->id]) }}" class="rform-back" title="Kembali ke Detail Baterai">
                ...
                <form action="{{ route('batteries.update', [$pop->id, $battery->id]) }}" method="POST" id="batteryForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="rform-section-body">
                            <div class="rform-row rform-row-2">
                                ...
                                <div class="rform-group">
                                    ...
                                    <input type="text" name="type_pop" class="rform-input @error('type_pop') is-invalid @enderror" value="{{ old('type_pop', $battery->type_pop ?? $pop->tipe_pop) }}" required>
                            ...
                            <div class="rform-row rform-row-2">
                                <div class="rform-group">
                                    ...
                                    <input type="text" name="building" class="rform-input @error('building') is-invalid @enderror" value="{{ old('building', $battery->building ?? $pop->jenis_bangunan) }}" required>
                                ...
                                <div class="rform-group">
                                    ...
                                    <input type="text" name="pic" class="rform-input @error('pic') is-invalid @enderror" value="{{ old('pic', $battery->pic ?? auth()->user()?->name) }}" required>
                        ...
                        <div class="rform-section-body">
                            <div class="rform-row rform-row-2">
                                <div class="rform-group">
                                    ...
                                    <select id="rectifier_id" name="rectifier_id" class="rform-select @error('rectifier_id') is-invalid @enderror" required onchange="handleRectifierChange()">
                                        ...
                                        @forelse ($rectifiers as $r)
                                            <option value="{{ $r->id }}"
                                                data-beban="{{ $r->beban }}"
                                                {{ (old('rectifier_id', $battery->rectifier_id) == $r->id) ? 'selected' : '' }}>
                                ...
                                <div class="rform-group">
                                    ...
                                    <input type="text" id="nomor_bank" name="nomor_bank" class="rform-input @error('nomor_bank') is-invalid @enderror" value="{{ old('nomor_bank', $battery->nomor_bank) }}" required>
                            ...
                            <div class="rform-row rform-row-2">
                                <div class="rform-group">
                                    ...
                                    <select id="merk_battery" name="merk_battery" class="rform-select @error('merk_battery') is-invalid @enderror" required onchange="updateTipeOptions()">
                                        <option value="SACRED SUN" {{ $currentMerk == 'SACRED SUN' ? 'selected' : '' }}>SACRED SUN</option>
                                        <option value="NARADA" {{ $currentMerk == 'NARADA' ? 'selected' : '' }}>NARADA</option>
                                        <option value="SHOTO" {{ $currentMerk == 'SHOTO' ? 'selected' : '' }}>SHOTO</option>
                                        <option value="COSLIGHT" {{ $currentMerk == 'COSLIGHT' ? 'selected' : '' }}>COSLIGHT</option>
                                        <option value="VISION" {{ $currentMerk == 'VISION' ? 'selected' : '' }}>VISION</option>
                                        <option value="LEOCH" {{ $currentMerk == 'LEOCH' ? 'selected' : '' }}>LEOCH</option>
                                        <option value="HUAWEI" {{ $currentMerk == 'HUAWEI' ? 'selected' : '' }}>HUAWEI</option>
                                        <option value="ZTE" {{ $currentMerk == 'ZTE' ? 'selected' : '' }}>ZTE</option>
                                        <option value="ENERSYS" {{ $currentMerk == 'ENERSYS' ? 'selected' : '' }}>ENERSYS</option>
                                        <option value="MAXLIFE" {{ $currentMerk == 'MAXLIFE' ? 'selected' : '' }}>MAXLIFE</option>
                                        <option value="OTHER" {{ !in_array($currentMerk, ['SACRED SUN','NARADA','SHOTO','COSLIGHT','VISION','LEOCH','HUAWEI','ZTE','ENERSYS','MAXLIFE']) ? 'selected' : '' }}>+ Merk Lainnya</option>
                                    </select>
                                    <input type="text" id="custom_merk_battery" class="rform-input" style="display:none; margin-top:8px;" value="{{ $currentMerk }}" placeholder="Ketik Merk Baterai Baru...">
                                ...
                                <div class="rform-group">
                                    ...
                                    <input type="text" id="custom_tipe_battery" class="rform-input" style="display:none; margin-top:8px;" value="{{ old('tipe_battery', $battery->tipe_battery) }}" placeholder="Ketik Tipe Baterai Baru...">
                            ...
                            <div class="rform-row rform-row-2">
                                <div class="rform-group">
                                    ...
                                    <select id="jenis_battery" name="jenis_battery" class="rform-select @error('jenis_battery') is-invalid @enderror" required>
                                        <option value="Lithium" {{ old('jenis_battery', $battery->jenis_battery) == 'Lithium' ? 'selected' : '' }}>Lithium</option>
                                        <option value="VRLA" {{ old('jenis_battery', $battery->jenis_battery) == 'VRLA' ? 'selected' : '' }}>VRLA</option>
                                    </select>
                                ...
                                <div class="rform-group">
                                    ...
                                    @php
                                        $currentKap = (int) old('kapasitas_battery', $battery->kapasitas_battery);
                                    @endphp
                                    <select id="kapasitas_battery" name="kapasitas_battery" class="rform-select @error('kapasitas_battery') is-invalid @enderror" required onchange="updateBatteryCalculation()">
                                        <option value="20" {{ $currentKap == 20 ? 'selected' : '' }}>20 AH</option>
                                        <option value="50" {{ $currentKap == 50 ? 'selected' : '' }}>50 AH</option>
                                        <option value="100" {{ $currentKap == 100 ? 'selected' : '' }}>100 AH</option>
                                        <option value="200" {{ $currentKap == 200 ? 'selected' : '' }}>200 AH</option>
                                    </select>
                            ...
                            <div class="rform-row rform-row-2">
                                <div class="rform-group">
                                    ...
                                    <input type="text" id="kapasitas_uji" name="kapasitas_uji" class="rform-input @error('kapasitas_uji') is-invalid @enderror" value="{{ old('kapasitas_uji', $battery->kapasitas_uji !== null ? str_replace('.', ',', (string)(float)$battery->kapasitas_uji) : '') }}" placeholder="Contoh: 91,67 atau 100" oninput="updateBatteryCalculation()">
                                ...
                                <div class="rform-group">
                                    ...
                                    <div style="position: relative;">
                                        <input type="text" id="kapasitas_battery_persen_display" class="rform-input" readonly placeholder="Otomatis" tabindex="-1" style="background:#f1f5f9; color:#334155; cursor:not-allowed; padding-right:110px;">
                                    ...
                                    <input type="hidden" name="kapasitas_battery_persen" id="kapasitas_battery_persen" value="{{ old('kapasitas_battery_persen', $battery->kapasitas_battery_persen) }}">
                        ...
                        <div class="rform-section-body">
                            <div class="rform-row rform-row-2">
                                <div class="rform-group">
                                    ...
                                    <input type="date" id="tanggal_uji_terakhir" name="tanggal_uji_terakhir" class="rform-input @error('tanggal_uji_terakhir') is-invalid @enderror" value="{{ old('tanggal_uji_terakhir', $battery->tanggal_uji_terakhir ? $battery->tanggal_uji_terakhir->format('Y-m-d') : '') }}" onchange="updateStatusUji()">
                                ...
                                <div class="rform-group">
                                    ...
                                    <input type="date" id="tanggal_penggantian" name="tanggal_penggantian" class="rform-input @error('tanggal_penggantian') is-invalid @enderror" value="{{ old('tanggal_penggantian', $battery->tanggal_penggantian ? $battery->tanggal_penggantian->format('Y-m-d') : '') }}">
                                </div>
                            </div>
                            ...
                            <div class="rform-row rform-row-2">
                                <div class="rform-group">
                                    ...
                                    <input type="hidden" name="status_uji" id="status_uji" value="{{ old('status_uji', $battery->status_uji ?? 'BLM UJI BATT') }}">
                                    <div style="position:relative;">
                                ...
                                <div class="rform-group">
                                    ...
                                    <input type="text" id="area_sti" name="area_sti" class="rform-input @error('area_sti') is-invalid @enderror" value="{{ old('area_sti', $battery->area_sti ?? $pop->kota_kabupaten) }}" required>
                    ...
                    <div class="rform-actions">
                        <a href="{{ route('batteries.show', [$pop->id, $battery->id]) }}" class="rform-btn-reset" style="text-decoration: none; display: inline-flex; align-items: center; justify-content: center;">Batal</a>
    ...
    <script>
        ...
        const existingTipe = "{{ old('tipe_battery', $battery->tipe_battery) }}";
        const existingMerk = "{{ old('merk_battery', $battery->merk_battery) }}";
        ...
        function updateTipeOptions() {
            ...
            listTipe.forEach(t => {
                ...
                opt.textContent = t;
                if (existingTipe && (existingTipe === t || (t === 'Custom...' && !listTipe.includes(existingTipe)))) {
                    opt.selected = true;
                }
        ...
        function handleTipeChange() {
            ...
            if (tipeSelect.value === 'TIPE_CUSTOM') {
                ...
                if (existingTipe && !tipeMap[document.getElementById('merk_battery').value]?.includes(existingTipe)) {
                    customTipeInput.value = existingTipe;
                }
            } else {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            if (existingMerk) {
                document.getElementById('merk_battery').value = existingMerk;
            }
            updateTipeOptions();
            updateBatteryCalculation();
            updateStatusUji();
